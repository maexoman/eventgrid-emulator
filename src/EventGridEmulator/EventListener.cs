...
namespace EventGridEmulator
{
	public class EventListener
	{
		...
		{
			var topicConfiguration = _requestParser.RetrieveTopicConfigurationFromUrl(request.RawUrl == null ? "" : request.RawUrl);
			...
			if (topicConfiguration != null)
			{
				...
				if (events != null && events.Count() > 0)
				{
					...
					{
						_logger.LogInfo($"Received event: {JsonSerializer.Serialize(ev)}");
					}
